    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center mb-8 animate-fade-in">
        ...
        <div class="flex gap-4 items-center">
          <span v-if="authStore.user" class="text-gray-700 dark:text-gray-300">
            Welcome, {{ authStore.user.username }}!
          </span>
          <button
            @click="handleLogout"
            class="px-4 py-2 bg-gray-200 text-gray-900 rounded-md hover:bg-gray-300 transition-colors text-sm/6 font-semibold dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600"
          >
            Logout
          </button>
          <button
      ...
      <div v-else-if="blogStore.error" class="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-md mb-6 dark:bg-red-500/10 dark:border-red-500/20 dark:text-red-400">
        {{ blogStore.error }}
      </div>

      <div v-else-if="blogStore.blogs.length === 0" class="text-center py-12">
        <p class="text-gray-500 dark:text-gray-400 text-lg">No blog posts yet. Create your first post!</p>
      </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
          ...
          :key="blog.id"
          class="bg-white rounded-md border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 animate-fade-in dark:bg-white/5 dark:border-white/10 dark:hover:bg-white/10"
        ...
        >
          <div class="p-6">
            ...
            <button
              ...
              class="w-full bg-indigo-600 text-white py-2 rounded-md hover:bg-indigo-500 transition-colors text-sm/6 font-semibold shadow-xs focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 dark:bg-indigo-500 dark:shadow-none dark:hover:bg-indigo-400 dark:focus-visible:outline-indigo-500"
            >
              Read More
            </button>
          </div>
